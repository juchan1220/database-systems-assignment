...
pagenum_t find_leaf(int64_t key)
{
	...
	while (!cur->is_leaf) {
		...
		pagenums = (pagenum_t *)cur->pointers;
		next_num = pagenums[i];
...
void print_tree(int table_id)
{
	...
	pagenum_t* pagenums;
	if (queue == NULL) {
		queue = (pagenum_t *)malloc(sizeof(int64_t) * 5000);
		queue_lv = (int *)malloc(sizeof(int) * 5000);
		queue_size = 5000;
	}
...
void init_page(page_t* new_page, bool isLeaf)
{
	if (isLeaf) {
		new_page->is_leaf = 1;
		new_page->keys = (int64_t *)malloc(sizeof(int64_t) * LEAF_ORDER);
		new_page->pointers = malloc(sizeof(record) * LEAF_ORDER);
	}
	else {
		new_page->is_leaf = 0;
		new_page->keys = (int64_t *)malloc(sizeof(int64_t) * INTERNAL_ORDER);
...
int get_left_index(page_t *parent, pagenum_t leftnum)
{
	...
	pagenums = (pagenum_t *)parent->pointers;
...
void insert_into_leaf_after_splitting(pagenum_t leafnum, int64_t key, const char* value)
{
	...
	temp_values = (record *)malloc(LEAF_ORDER * sizeof(record));
	...
	records = (record *)new_leaf->pointers;
...
{
	int i;
	pagenum_t* pagenums = (pagenum_t *)parent->pointers;
...
void insert_into_node_after_splitting(page_t* parent, pagenum_t parentnum, int left_index, int64_t key, pagenum_t rightnum)
{
	...
	temp_pagenums = (pagenum_t *)malloc((INTERNAL_ORDER + 1) * sizeof(pagenum_t));
	...
	pagenums = (pagenum_t *)new_node->pointers;
...
void insert_into_new_root(page_t* left, pagenum_t leftnum, int64_t key, page_t* right, pagenum_t rightnum)
{
	...
	pagenums = (pagenum_t *)root->pointers;
...
void merge_nodes(page_t* node, pagenum_t node_num, page_t* neighbor, pagenum_t neighbor_num, int neighbor_index, int64_t k_prime)
{
	...
	if (node->is_leaf == 0) {
		...
		pagenums1 = (pagenum_t *)neighbor->pointers;
		pagenums2 = (pagenum_t *)node->pointers;
	...
	// Case : Leaf node
		...
		records1 = (record *)neighbor->pointers;
		records2 = (record *)node->pointers;
...
void redistribute_nodes(page_t* node, pagenum_t node_num, page_t* neighbor, pagenum_t neighbor_num, int neighbor_index, int k_prime_index, int64_t k_prime)
{
	...
	if (neighbor_index != -1) {
		if (node->is_leaf == 0) {
			pagenums1 = (pagenum_t *)node->pointers;
			pagenums2 = (pagenum_t *)neighbor->pointers;
			pagenums1[node->num_keys + 1] = pagenums1[node->num_keys];
		}
		else {
			records1 = (record *)node->pointers;
			records2 = (record *)neighbor->pointers;
		} 
...
void remove_entry_from_node (page_t* node, pagenum_t node_num, int64_t key, void* pointers)
{
	...
	// Case : internal node, find child's pagenum and delete it
		...
		pagenums = (pagenum_t *)node->pointers;
